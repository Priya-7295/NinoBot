module.exports.config = {
    ...
    commandCategory: "Giao Tiếp",
    usages: "ninoteach câu muốn hỏi nino => câu muốn nino trả lời",
    cooldowns: 5
};
    ...
    if (fw == -1) {
        ...
        if (ask=="") {api.sendMessage("thiếu câu hỏi kìa ;-;",threadID,messageID)} else {
            if (!answer) {api.sendMessage("thiếu câu trả lời kìa ;-;",threadID,messageID)} else {
                    axios.get(encodeURI(`https://www.api-adreno.tk/nino/add/${ask}&&${answer}`)).then(res => {
