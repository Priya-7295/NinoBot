module.exports.config = {
	...
	description: "Cảnh cáo thành viên  đủ 3 lần sẽ bị kick khỏi Nhóm",
	...
	info: [
		{
			...
			type: '',
			example: 'canhcao [tag] "lý do cảnh cáo"'
  		},
...

		{
			...
			type: '',
			example: 'canhcao unban [id của user cần xóa]'
  		},
		{
			key: 'view',
			...
			type: '',
			example: 'canhcao view [@tag] / canhcao view'
  		},

		{
			key: 'reset',
			prompt: 'Reset toàn bộ dữ liệu canhcao trong nhóm của bạn',
			type: '',
			example: 'canhcao reset'
  		}
...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
  ...
  {canhcao: {}, banned: {tid: []}};
  */
  if(!datawarn.canhcao.hasOwnProperty(threadID)) {
			datawarn.canhcao[threadID] = {}; 
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		var msg = "";
  		var mywarn = datawarn.canhcao[threadID][senderID];
  		...
  		for(let id of mentions) {
  			...
  			var msg = "";
  			var so = 1;
  			var reasonarr = datawarn.canhcao[threadID][id];
  		...
  		var dtwbox = datawarn.canhcao[threadID];
  		var allwarn = "";
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Chỉ qtv nhóm mới có thể sử dụng lệnh unban!', threadID, messageID);
  	...
  	if(!mybox.includes(id)) return api.sendMessage("✅Người này chưa bị cấm vào nhóm của bạn", threadID, messageID);
			...
			delete datawarn.canhcao[threadID][id]
			fs.writeFileSync(__dirname + `/cache/datawarn.json`, JSON.stringify(datawarn, null, 2));
  }
  ...
  var reason = "";
		...
		for(let iid of iduser) {
			...
			var dtwmybox = datawarn.canhcao[threadID];
			if(!dtwmybox.hasOwnProperty(id)) { 
			dtwmybox[id] = [];
			}
			var solan = (datawarn.canhcao[threadID][id]).length;
			arrayname.push(nametag+" lần "+(parseInt(solan)+1));
			var pushreason = datawarn.canhcao[threadID][id];
			pushreason.push(reason);
			if(!datawarn.banned[threadID]) {
				datawarn.banned[threadID] = [];
			}
			if((datawarn.canhcao[threadID][id]).length > 2) {
