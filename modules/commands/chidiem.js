...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        var one  = mention[0];
        return makeImage({ one }).then(path => api.sendMessage({ body: event.mentions[mention[0]].replace("@", "") + " đã bị chỉ", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}