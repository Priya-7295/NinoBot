module.exports.config = {
	...
	description: "Fact của Donald Trump",
...
module.exports.run = async function({ api, event, args }) {
    ...
    let pathImg = __dirname + '/cache/fact.png';
	var text = args.join(" ");
	let name = (await api.getUserInfo(senderID))[senderID].name
	var linkAvatar = (await api.getUserInfo(senderID))[senderID].thumbSrc;
	if (!text) return api.sendMessage("Bạn chưa nhập nội dung Fact.", threadID, messageID);
	let getAvatar = (await axios.get(linkAvatar, { responseType: 'arraybuffer' })).data;
	let getPorn = (await axios.get(`https://i.imgur.com/G10IegL.jpg`, { responseType: 'arraybuffer' })).data;
